...
/**
 * Classe ControleEmpresa do pacote controle. Manipula os dados da Model para
 * utilizar nas telas do pacote View.
 * 
...
public class ControleEmpresa {
  ...
  /**
   * CRUD
   * Cadastra uma filial e percorre todas as filiais ja cadastradas no sistema,
   * verificando se
   ...
   * E depois a filial cadastrada e inserida dentro do ArrayList de Filiais da
   * Classe Empresa.
   * 
   * @param dadosFilial[]
   * @param pos
  ...
  public boolean cadastroFilial(String dadosFilial[], int pos) {
    ...
    for (int i = 0; i < dados.getFiliais().size(); i++) {
      if (i == pos) {
        ArrayList<Produto> anteriores = dados.getFiliais().get(pos).getProdutodeEstoque();
        dados.getFiliais().set(pos, filial);

        dados.getFiliais().get(pos).setProdutodeEstoque(anteriores);
        return true;
      }
    }
   ...
   * @param nomeProduto
   * @return int[]
   *         O metodo comeca criando um array de inteiros posicoes com tres
   *         posicoes.
   *         Em seguida, ele percorre todas as filiais cadastradas no sistema e,
   *         para cada filial,
   *         percorre a lista de produtos em estoque. Se o nome do produto for
   *         igual ao nome passado como parametro,
   *         o metodo preenche as posicoes do array posicoes com a posicao da
   *         filial
   *         e a posicao do produto na lista de produtos da filial.
   *         Alem disso, ele verifica se o produto e uma instancia da classe
   *         Alimento
   *         ou da classe Bebida e preenche a terceira posicao do array posicoes
   *         com o valor 1 ou 2,
  ...
  /**
   * Metodo criado para preencher os dados pre cadastrados para colocar na Jlist
   * da Tela Produto
   * do pacote View.
   * Nesse metodo especifico foi preciso converter as classes Alimento e Bebida na
   * sua classe pai.
   * Foi criada uma instancia da classe Produto e foram colocados exemplos de
   * atributos.
